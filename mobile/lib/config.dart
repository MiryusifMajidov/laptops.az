...
class AppConfig {
  ...
  /// Kart/siyahı üçün kiçildilmiş, sıxılmış şəkil (backend /img proksisi).
  /// w<=0 → tam ölçü. Mənbə həm "/uploads/..", həm xarici link ola bilər.
  static String thumb(String path, {int w = 500}) {
    if (path.isEmpty) return '';
    if (w <= 0) return img(path);
    return '$host/img?u=${Uri.encodeComponent(path)}&w=$w';
  }
}
