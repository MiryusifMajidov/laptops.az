...
const _welcome =
    'Salam! 👋 Sizə uyğun notebook tapmaqda kömək edim. Nə üçün istifadə edəcəksiniz?';

const List<List<String>> _quick = [
  ['🎮 Oyun', 'Oyun üçün notebook axtarıram, tövsiyə edin'],
  ['💼 İş', 'İş və ofis işləri üçün notebook lazımdır'],
  ['🎨 Dizayn', 'Dizayn və video montaj üçün güclü notebook'],
  ['🎓 Tələbə', 'Tələbə üçün büdcəyə uyğun notebook'],
];

class _AiChatScreenState extends State<AiChatScreen> {
  final _msgs = <_ChatMsg>[_ChatMsg('assistant', _welcome)];
  ...
  Future<void> _send(String raw) async {
    ...
    try {
      ...
      setState(() => _msgs.add(_ChatMsg('assistant',
          'Bağışlayın, bir problem oldu. Bir azdan yenidən yoxlayın 🙏')));
    } finally {
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text('AI Köməkçi', style: mr(size: 15.5, weight: FontWeight.w800)),
                Text('Onlayn · dərhal cavablayır',
                    style: mr(
  ...
  List<Widget> _assistantParts(_ChatMsg m) {
    ...
    for (final match in re.allMatches(m.text)) {
      final t = _clean(m.text.substring(last, match.start));
      if (t.isNotEmpty) out.add(_textBubble(t));
