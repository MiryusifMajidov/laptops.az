...
const SRC: Record<string, [string, string]> = { web: ['Sayt', 'ink'], app: ['Tətbiq', 'good'], whatsapp: ['WhatsApp', 'good'], instagram: ['Instagram', 'warn'] }
  ...
  return (
    <div className="content">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        {['all', 'web', 'app', 'whatsapp', 'instagram'].map((f) => (
          <span key={f} className={'chip' + (filter === f ? ' on' : '')} onClick={() => setFilter(f)}>
            {f === 'all' ? 'Hamısı' : (SRC[f]?.[0] ?? f)}
          </span>
        ))}
